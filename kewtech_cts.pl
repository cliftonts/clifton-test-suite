...
#For wishlist see primetest_cts

#Subroutines go here
sub help {
   print "Clifton Test Suite v1.0 - 2015 \n";
   print "Kewtech series - CTS Custom Format\n\n";
   print "Correct usage:\n";
   print "kewtech_cts --i <port or filename> --o <output file[.csv]>\n";
   print "--h = Help screen\n";
...
#do {
    ...
    for( $a = 0; $a < scalar(@DataStore); $a++ ){
       ...
       if ($tmp eq 'TEST NUMBER' or $tmp eq 'END OF DATA') {
          ...
          #print $final . "\n";
          if ($app_no ne '') {
             $final = $app_no . "," . $Date . "," . $location . "," . $description . "," . $earth . "," . $insulation . "," . $cont . "," . $site . "," . $comments . "," . $comment2;
             print OUTPUT $final . "\x0d\x0a";
          } else {
             print OUTPUT "ID,Date,Location,Description,Earth,Insulation,Continuity,Site,Comment 1,Comment 2\x0d\x0a";
          }
          $Count++;
          $Date = '';
          $time = '';
