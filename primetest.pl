...
sub help {
   ...
   print "Seaward Primetest series\n\n";
   print "Correct usage:\n";
   print "primetest --i <port or filename> --o <output file[.csv]>\n";
   print "--h = Help screen\n";
...
#do {
    ...
    for( $a = 0; $a < scalar(@DataStore); $a++ ){
       ...
       if ($tmp eq 'TEST NUMBER' or $tmp eq 'END OF DATA') {
          ...
          #print $final . "\n";
          if ($app_no ne '') {
             $final = $app_no . "," . $Date . "," . $time . "," . $site . "," . $description . "," . $visual . "," . $earth . "," . $earth_pass . "," . $insulation . "," . $insulation_pass . "," . $load . "," . $load_pass . "," . $leakage . "," . $leakage_pass . "," . $cont . "," . $comment1 . "," . $comment2 . "," . $comment3;
             print OUTPUT $final . "\x0d\x0a";
          } else {
             print OUTPUT "ID,Date,Time,Location,Description,Visual,Earth,Earth Pass,Insulation,Insulation Pass,Load,Load Pass,Leakage,Leakage Pass,Load,Load Pass,Continuity,Comment 1,Comment 2,Comment 3\x0d\x0a";
          }
          $Count++;
          $Date = '';
          $time = '';
